...
const schema = z.object({
  ...
  password: z.string().min(1, "Enter password").max(72),
});
    ...
    <div className="min-h-screen grid md:grid-cols-2 bg-background">
      <div className="hidden md:flex flex-col justify-between bg-gradient-soft p-10">
        ...
        <div className="space-y-4 max-w-md">
          ...
          <p className="text-muted-foreground">Pick up where you left off — your customers are waiting to be heard.</p>
        </div>
      ...
      <div className="flex items-center justify-center p-6 md:p-10">
        <Card className="w-full max-w-md p-8 border-border shadow-card">
          ...
          <p className="text-sm text-muted-foreground mt-1">Welcome back to Feedback Pro.</p>
          ...
          <form onSubmit={submit} className="space-y-4">
            ...
            <Button type="submit" className="w-full bg-gradient-cta shadow-glow" disabled={loading}>{loading ? "Logging in…" : "Log in"}</Button>
          </form>
          <p className="text-sm text-muted-foreground text-center mt-5">
            Don't have an account? <Link to="/signup" className="text-primary font-medium">Sign up free</Link>
          </p>
        </Card>
      </div>
    </div>
